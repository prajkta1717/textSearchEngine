...
@app.route('/')
def hello_world():
    return render_template('imageCaptioningSearch.html')

@app.route('/upload/<filename>')
def send_image(filename):
    return send_from_directory("images1",filename="image"+filename+".jpg")
...
def classification():
    if request.method == 'POST':
        contentOfIDF = open('TF_IDF-image.txt', 'r').read()
        ...
        return render_template("imageCaptioningResult.html", content=sortedResult)
